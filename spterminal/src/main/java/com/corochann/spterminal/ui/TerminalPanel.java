...
public class TerminalPanel extends DropPanel implements ActionListener {
    ...
    private void updateSelectedSuggestionIndex(int index) {
        ...
        if (index >= 0) {
            try {
                System.out.println("setCaretPosition to " + mTXTextPane.getLineEndOffset(currentSelectedSuggestionIndex) + ", " + currentSelectedSuggestionIndex);
                ...
                public void run(){
                    try {
                        ...
                        if (currentViewState == VIEW_STATE_FILTERED_LOG) {
                            ...
                            mRXFilteredTextPane.flushWithDelay(FLUSH_WITH_DELAY_MILLIS);
                        }
    ...
    private synchronized Object[][] constructFilterCountTableData() {
        Vector<FilterRule.FilterRuleElement> vec =
                currentFilterRule == null ? new Vector<FilterRule.FilterRuleElement>() : currentFilterRule.getFilterRuleVec();
    ...
    private class FilteredTextPaneDocumentFilter extends DocumentFilter {
        ...
        public void insertString(FilterBypass fb, int offset, String string, AttributeSet attr) throws BadLocationException {
            ...
            /* Filter before insertString */
            int newOffset = 0;
            String filteredString = "";
            int LFIndex = string.indexOf('\n', newOffset);
            while (LFIndex >= 0) {
                String lineStr = string.substring(newOffset, LFIndex+1);
                int startIndex = filter(lineStr);
                if (startIndex >= 0) { // filtered
                    filteredString += lineStr.substring(0, startIndex);
                } else { // not filtered
                    filteredString += lineStr;
                }
                newOffset = LFIndex+1;
                LFIndex = string.indexOf('\n', newOffset);
            }
            filteredString += string.substring(newOffset, string.length());
            //System.out.println("[DEBUG] FilteredTextPane filteredString: " + MyUtils.unEscapeString(filteredString));

            //super.insertString(fb, offset, string, attr);
            super.insertString(fb, offset, filteredString, attr);
            /* Filter after insertString
             * (to check if existing log and newly inserted log was the target of filter after combined) */
            ...
            try {
                ...
                for (int line = lineEnd; line >= lineStart; line--) {
                    ...
                    if (lineStr.contains("\n")) {  // remove line only after confirmed that lineStr is already completed sentence
                        int startIndex = filter(lineStr);
                        if (startIndex >= 0) {
                            //System.out.println("[DEBUG] FilteredTextPane removing from document: " + MyUtils.unEscapeString(lineStr) + ", from " + startIndex);
                            fb.remove(lineStartOffset + startIndex, lineStr.length() - startIndex); // remove until the end of line
                        }
                    }
                }
            } catch (BadLocationException ble) {
         ...
         * @param lineStr
         * @return the index to start remove. (should be filetered until the end of line)
         *         -1 when this lineStr should not be filtered
         */
        int filter(String lineStr) {
            //System.out.println("[DEBUG] filtering lineStr = " + MyUtils.unEscapeString(lineStr));
            if (currentFilterRule == null) return -1;
                ...
                if (ruleType == FilterRule.FilterRuleElement.RULE_TYPE_CONTAIN) {
                    if (regex) {
                        ...
                        if (pattern != null) {
                            ...
                            if (matcher.find()) pos = 0;
                        } else {
                            // skip this filter element
                        }
                    } else {
                        if (matchCase) {
                            pos = lineStr.contains(query) ? 0 : -1;
                        } else {
                            pos = lineStr.toLowerCase().contains(query.toLowerCase()) ? 0 : -1;
                        }
                    }
                } else if (ruleType == FilterRule.FilterRuleElement.RULE_TYPE_STARTSWITH) {
                    if (regex) {
                        Pattern pattern = getPattern(query, matchCase, false);
                        if (pattern != null) {
                            Matcher matcher = pattern.matcher(lineStr);
                        ...
                        if (matchCase) {
                            pos = lineStr.indexOf(query);
                        } else {
                            pos = lineStr.toLowerCase().indexOf(query.toLowerCase());
                        }
                    }
                } else {
